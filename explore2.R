...
#               Class_9     152      10       4       2       2      74       6      32    1465
        ...
        ## has a kaggle score of 0.5657 - right direction but still not good enough
        
        
        
## LOOK AT Class_2 and Class_3
        ##make sure we run the above pply stuff
        
        ## use a subset for plotting sanity
        set.seed(8675309)
        long_train_x<-long_train[sample(1:nrow(long_train), 1500),]
        ## keep only values above 0
        long_train_x<-long_train_x[long_train_x$data>0,]
        ## compare 2&3
        p <- ggplot(long_train_x[long_train_x$target=="Class_3"|long_train_x$target=="Class_2",], aes(x=feature, y=data, color=target))+geom_point(size=3)+ theme_bw()
        p <- p + ggtitle("Class_2 and Class_3")
        print(p)       
        
        ## for reference compare 5&8
        p <- ggplot(long_train_x[long_train_x$target=="Class_5"|long_train_x$target=="Class_8",], aes(x=feature, y=data, color=target))+geom_point(size=3)+ theme_bw()
        p <- p + ggtitle("Class_5 and Class_8")
        print(p) 
        
        ## clearly 2&3 are not clean but nothing obvious
        
        ## let's look at class2 versus class3
        
        td_23<-train_data[train_data$target=="Class_3"|train_data$target=="Class_2",]
        ## use a subset for plotting sanity
        set.seed(8675309)
        td_23<-td_23[sample(1:nrow(td_23), 1000),]
        
       
#############################################
        ### TRY GBM9 with an added RF
        ## USE {gbm} PACKAGE
